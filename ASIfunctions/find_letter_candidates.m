function [letter_candidates]=find_letter_candidates(audiofolder,files,P1,P2,n,best_match_from_same_file)
...
for i=1:n
    ...
    while cont
        ...
        if sum(isnan(data(:)))+sum(isnan(reference_data(:)))==0
            ...
            x2=x1+dx-1;
            letter=data(y1:y2,x1:x2);
            if var(letter(:))>10^(-15)
                correl=compute_correlations(reference_data, letter, [y1 y2], P1.tolerance);
                [ma , ~]=max(correl);
                if reference_recording==recording
                    ma(x1:x2)=0;
                end
                [maxcor , ~]=max(ma);
                si=sum(letter(:)>0.8*max(letter(:)));
                if and(maxcor>P2.correlation_threshold,si>P2.complexity_threshold)
